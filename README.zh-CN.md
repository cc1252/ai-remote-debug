...
发现并锁定正确的在线设备
    ↓
收集日志、进程、服务、配置、网络和运行环境等最小必要证据
    ↓
建立原因假设，并用针对性检查排除其他可能
    ↓
向工程师报告结论和证据
    ↓
获得明确授权后执行最小修复
...
| 客户问题 | AI 使用 Skill 后可以做什么 |
|---|---|
| Windows 软件无法启动或闪退 | 检查进程、服务、事件日志、依赖、配置文件和端口占用，定位启动失败的位置 |
| 开发机正常、客户机异常 | 对比系统版本、运行库、环境变量、目录权限、网络和外部依赖，查出环境差异 |
| Android 应用报错 | 读取设备信息、聚焦 logcat、查看 dumpsys 和应用状态，把异常关联到具体包与进程 |
| 手机黑屏或进入 recovery/fastboot | 通过旁边的客户电脑确认 ADB/fastboot 状态，在执行重启、刷写等操作前明确说明风险 |
| 已经找到修复方案 | 给出准确的目标、命令、影响和回退方式，取得授权后执行，并用原始故障场景复验 |
...
```text
使用 $ai-remote-debug 检查“客户A-收银台电脑”的 MyApp 服务和今天的错误日志。
允许重启 MyApp 服务，但不要修改配置、安装软件或重启电脑。完成后验证服务状态并汇报证据。
```

Skill 同时支持 MCP 和 `ard` CLI 回退，并内置了客户授权、敏感数据最小化、命令确认、ADB/fastboot 高风险操作等边界。Skill 本身不保存 Relay token、不会绕过 MCP 客户端权限，也不会让 AI 自动获得任何客户设备的访问权；它只能使用操作者已经配置并明确授权的连接。
